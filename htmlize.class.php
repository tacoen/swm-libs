...
/* -- forms ------------------------------------------------------------------------------------------------ */
	...
	public function form_query($container,$el='input',$name,$type='text',$func,$label=NULL,$option=array(),$array_passed=array() ) {
		...
		switch($el) {
			case 'select':		$html .= $this->fq_select($name,$val,$option); break;
			case 'textarea':	$html .="<textarea name='$name'>$val</textarea>"; break;
			default:			$html .=$this->fq_input($name,$val,$type); break;
		}
