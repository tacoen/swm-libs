...
class htmlize {
	...
	public function query($w,$s) { $this->q[$w]['html'] .= $s; }

	public function build($w) {
		
		if ( isset($this->q[$w])) { $el = $this->q[$w]; }
		else if ( isset($this->a[$w])) { $el = $this->a[$w]; }
		else { $el = array(); }
	...
	public function append($w,$s) { $this->a[$w]['html'] .=$s; }

	public function add($w, $func=NULL, $id=NULL, $attr=NULL, $array_passed = array() ) {
		if (isset($id))   { $this->a[$w]['id'] = $id; }
		if (isset($attr)) { $this->a[$w]['attr'] = $attr; }
		$html = '';
		if (isset($func)) { 
...
class formize {
	...
	public function input($name,$val,$type) {
		switch($type) {
			case 'submit':
				...
				break;
			case 'checkbox':
				$str = "<input type='$type' name='$name' value='$val' /> $val";
				break;
			case 'radio':
				$str = "<input type='$type' name='$name' value='$val' /> $val";
				break;
			default:
				$str = "<input type='$type' name='$name' value='$val' />";
				break;
		}
		return $str;
	}
	...
	public function add($set,$name,$el,$val,$type='text',$label=NULL,$option=array(),$order = NULL) {
		...
		switch($el) {
			case 'select':
				$str .= $this->select($name,$val,$option);
				break;
			case 'textarea':
				$str .= "<textarea name='$name'>$val</textarea>";
				break;
			default:
				$str .= $this->input($name,$val,$type);
				break;
		}
		
		$this->field[$set][$name]['element'] = $str;
...
class stylize {
	...
	public function add($obj,$url,$media,$n = NULL) {
...
class scriptize {
	...
	public function add($obj,$url,$n = NULL, $after=false) {
		if (!$n) { $order = $this->order; } else { $order=$n; }
